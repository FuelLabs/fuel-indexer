...
pub async fn put_object(conn: &mut PoolConnection<Sqlite>, query: String, bytes: Vec<u8>) -> sqlx::Result<usize> {
...
pub async fn root_columns_list_by_id(conn: &mut PoolConnection<Sqlite>, root_id: i64) -> sqlx::Result<Vec<RootColumns>> {
    let query = format!(
        r#"SELECT
...
pub async fn new_root_columns(conn: &mut PoolConnection<Sqlite>, cols: Vec<NewRootColumns>) -> sqlx::Result<usize> {
    let mut builder = sqlx::QueryBuilder::new("INSERT INTO graph_registry_root_columns (root_id, column_name, graphql_type)");
...
pub async fn new_graph_root(conn: &mut PoolConnection<Sqlite>, root: NewGraphRoot) -> sqlx::Result<usize> {
    let mut builder = sqlx::QueryBuilder::new("INSERT INTO graph_registry_graph_root (version, schema_name, query, schema) VALUES (?, ?, ?, ?)");
...
pub async fn graph_root_latest(conn: &mut PoolConnection<Sqlite>, name: &str) -> sqlx::Result<GraphRoot> {
    let query = format!("SELECT * FROM graph_registry_graph_root WHERE schema_name = '{}' ORDER BY id DESC LIMIT 1", name);
...
pub async fn type_id_list_by_name(conn: &mut PoolConnection<Sqlite>, name: &str, version: &str) -> sqlx::Result<Vec<TypeId>> {
...
pub async fn type_id_latest(conn: &mut PoolConnection<Sqlite>, schema_name: &str) -> sqlx::Result<String> {
    let query = format!("SELECT schema_version FROM graph_registry_type_ids WHERE schema_name = '{}' ORDER BY id", schema_name);
    let row = sqlx::query(&query).fetch_one(conn).await?;

    Ok(row.get(0))
}

pub async fn type_id_insert(conn: &mut PoolConnection<Sqlite>, type_ids: Vec<TypeId>) -> sqlx::Result<usize> {
...
pub async fn schema_exists(conn: &mut PoolConnection<Sqlite>, name: &str, version: &str) -> sqlx::Result<bool> {
...
pub async fn new_column_insert(conn: &mut PoolConnection<Sqlite>, cols: Vec<NewColumn>) -> sqlx::Result<usize> {
...
pub async fn list_column_by_id(conn: &mut PoolConnection<Sqlite>, col_id: i64) -> sqlx::Result<Vec<Columns>> {
...
pub async fn columns_get_schema(conn: &mut PoolConnection<Sqlite>, name: &str, version: &str) -> sqlx::Result<Vec<ColumnInfo>> {
