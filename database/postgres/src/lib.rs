...
use sqlx::{Postgres, pool::PoolConnection, PgConnection};
...
pub async fn execute_query(conn: &mut PoolConnection<Postgres>, query: String) -> sqlx::Result<usize> {
    let mut builder = sqlx::QueryBuilder::new(query);
...
pub async fn root_columns_list_by_id(conn: &mut PoolConnection<Postgres>, root_id: i64) -> sqlx::Result<Vec<RootColumns>> {
...
pub async fn new_root_columns(conn: &mut PoolConnection<Postgres>, cols: Vec<NewRootColumns>) -> sqlx::Result<usize> {
...
pub async fn graph_root_latest(conn: &mut PoolConnection<Postgres>, name: &str) -> sqlx::Result<GraphRoot> {
    sqlx::query_as!(GraphRoot, "SELECT * FROM graph_registry_graph_root WHERE schema_name = $1 ORDER BY id DESC LIMIT 1", name).fetch_one(conn).await
}

pub async fn type_id_list_by_name(conn: &mut PoolConnection<Postgres>, name: &str, version: &str) -> sqlx::Result<Vec<TypeId>> {
...
pub async fn type_id_latest(conn: &mut PoolConnection<Postgres>, schema_name: &str) -> sqlx::Result<String> {
    let latest = sqlx::query_as!(IdLatest, "SELECT schema_version FROM graph_registry_type_ids WHERE schema_name = $1 ORDER BY id", schema_name).fetch_one(conn).await?;

    Ok(latest.schema_version)
}

pub async fn type_id_insert(conn: &mut PoolConnection<Postgres>, type_ids: Vec<TypeId>) -> sqlx::Result<usize> {
...
pub async fn schema_exists(conn: &mut PoolConnection<Postgres>, name: &str, version: &str) -> sqlx::Result<bool> {
...
pub async fn new_column_insert(conn: &mut PoolConnection<Postgres>, cols: Vec<NewColumn>) -> sqlx::Result<usize> {
...
pub async fn list_column_by_id(conn: &mut PoolConnection<Postgres>, col_id: i64) -> sqlx::Result<Vec<Columns>> {
...
pub async fn columns_get_schema(conn: &mut PoolConnection<Postgres>, name: &str, version: &str) -> sqlx::Result<Vec<ColumnInfo>> {
    sqlx::query_as!(ColumnInfo,
        r#"SELECT
       ...
       name, version).fetch_all(conn).await
}
