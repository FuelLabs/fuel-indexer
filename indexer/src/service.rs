use crate::{
    handler::Handle, CustomIndexExecutor, Executor, IndexerResult, Manifest, NativeHandler,
...
impl IndexerService {
    ...
    pub fn add_native_indexer(
        ...
        let mut executor = CustomIndexExecutor::new(&self.database_url.clone(), manifest.clone())?;

        for handle in handles {
            executor.register(NativeHandler {
                event: "foo".to_owned().into(),
