...
pub fn consume_definition<'a, S>(
    s: &'a str,
) -> Result<(Definition<'a, S>, &'a str), ParseError>
where
    S: Text<'a>,
{
...
mod test {
    ...
    fn one_field() {
        assert_eq!(
            ast("{ a }"),
            Document {
                definitions: vec![Definition::Operation(
    ...
    fn consume_single_query() {
        let (query, remainder) =
            consume_definition::<String>("query { a } query { b }").unwrap();
