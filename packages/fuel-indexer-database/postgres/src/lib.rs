...
pub async fn foreign_key_insert(
    ...
    let mut builder = sqlx::QueryBuilder::new("INSERT INTO graph_registry_foreign_keys (schema_version, schema_name, schema_identifier, table_name, column_name, reference_table_name, reference_column_name, reference_column_type, db_type, namespace, on_delete, on_update)");

    builder.push_values(foreign_keys.into_iter(), |mut b, foreign_key| {
        b.push_bind(schema_version)
            .push_bind(schema_name)
            .push_bind(schema_identifier)
...
pub async fn foreign_key_list_by_name(
    ...
    let query = format!("SELECT table_name, column_name, reference_table_name, reference_column_name, reference_column_type, namespace, db_type, on_delete, on_update FROM graph_registry_foreign_keys WHERE schema_version = '{schema_version}' AND schema_name = '{schema_name}' AND schema_identifier = '{schema_identifier}'");

    let rows = sqlx::query(&query).fetch_all(conn).await.unwrap();
