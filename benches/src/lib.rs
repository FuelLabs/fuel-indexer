...
async fn setup_wasm_executor(
    ...
    let config = IndexerConfig {
        database: DatabaseConfig::from_str(&db_url).unwrap(),
        ..Default::default()
    };
...
/// Create a function that for use in benchmarking a WASM indexer.
...
pub fn create_wasm_indexer_benchmark(
    ...
    num_blocks: usize,
    name: &str,
) -> impl Fn(&mut Criterion, Manifest) + '_ {
    move |c: &mut Criterion, manifest: Manifest| {
