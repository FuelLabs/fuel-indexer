...
use crate::{get_schema_types, schema_version, type_id, BASE_SCHEMA};
use fuel_indexer_database::{
    queries, DbType, IndexerConnection, IndexerConnectionPool, IndexerDatabaseError,
};
...
impl SchemaManager {
    pub async fn new(
        db_conn: impl Into<String>,
    ) -> Result<SchemaManager, IndexerDatabaseError> {
        let pool = IndexerConnectionPool::connect(&db_conn.into()).await?;

        Ok(SchemaManager { pool })
    }

    pub async fn new_schema(&self, name: &str, schema: &str) -> Result<(), sqlx::Error> {
    ...
    pub async fn load_schema_wasm(&self, name: &str) -> Result<Schema, sqlx::Error> {
        // TODO: might be nice to cache this data in server?
        Schema::load_from_db(&self.pool, name).await
    }
}
